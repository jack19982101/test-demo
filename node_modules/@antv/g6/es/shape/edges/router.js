...
export var pathFinder = function pathFinder(startPoint, endPoint, startNode, endNode, routerCfg) {
  ...
  var map = getObstacleMap(cfg.obstacles.concat([startNode, endNode]), cfg.gridSize, cfg.offset); // console.log(map);
  ...
  var endPoints = getBoxPoints(scaleEndPoint, endNode, cfg); // console.log(startPoints, startNode.getBBox(), endNode.getBBox(), endPoints);
  ...
  var _loop_1 = function _loop_1() {
    ...
    if (!current) return "break"; // console.log('cur: ', current.id, curCost, gScore[current.id]);
    // 如果 fScore 最小的点就是终点
    ...
    for (var i = 0; i < cfg.directions.length; i++) {
      ...
      var costFromStart = gScore[current.id] + neighborCost; // console.log(current.id, neighbor.id, directionChange);
